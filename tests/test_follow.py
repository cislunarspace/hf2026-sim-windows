...
try:
    ...
    pytest.skip("algorithms.tracking.follow 尚未实现，跳过测试", allow_module_level=True)
...
class TestComputeLeadPoint:
    ...
    def test_static_target(self):
        ...
        lead_lat, lead_lon = compute_lead_point(
            ...
            target_v_east=0.0, target_v_north=0.0,
            lead_time_s=2.0,
        )
    ...
    def test_eastbound_target(self):
        ...
        lead_lat, lead_lon = compute_lead_point(
            target_lat=27.005, target_lon=125.005,
            target_v_east=8.0, target_v_north=0.0,
            lead_time_s=2.0,
        )
    ...
    def test_northbound_target(self):
        ...
        lead_lat, lead_lon = compute_lead_point(
            target_lat=27.005, target_lon=125.005,
            target_v_east=0.0, target_v_north=8.0,
            lead_time_s=2.0,
        )
    ...
    def test_longer_lead_time(self):
        """更长的 lead time → 更大的偏移。"""
        lead_lat_1, lead_lon_1 = compute_lead_point(
            target_lat=27.005, target_lon=125.005,
            target_v_east=8.0, target_v_north=0.0,
            lead_time_s=1.0,
        )
        lead_lat_2, lead_lon_2 = compute_lead_point(
            target_lat=27.005, target_lon=125.005,
            target_v_east=8.0, target_v_north=0.0,
            lead_time_s=3.0,
        )
    ...
    def test_zero_lead_time(self):
        ...
        lead_lat, lead_lon = compute_lead_point(
            target_lat=27.005, target_lon=125.005,
            target_v_east=8.0, target_v_north=8.0,
            lead_time_s=0.0,
        )
        assert abs(lead_lat - 27.005) < 1e-6
        assert abs(lead_lon - 125.005) < 1e-6
