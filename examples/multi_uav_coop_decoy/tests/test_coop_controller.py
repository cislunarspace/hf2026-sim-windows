...
def test_track_mode_enters_after_consecutive_detections():
    ...
    state = _make_state(
        detected=True, sim_time=0.1, target_type="ground_vehicle", misid_flag=False
    )
...
def test_track_mode_broadcasts_tracking_message():
    ...
    s2 = _make_state(
        detected=True, sim_time=1.2, target_type="ground_vehicle", misid_flag=False
    )
...
def test_sector_search_fans_uavs_apart():
    ...
    base_cfg = {
        ...
        "sector_center_latitude": 27.0,
        "sector_center_longitude": 125.0,
        "sweep_pitch_min": -60.0,
        "sweep_pitch_max": -30.0,
    }
    c0 = CoopController(my_uid="20001")
    c0.configure(base_cfg)
    c0.reset()
    c1 = CoopController(my_uid="20002")
    c1.configure(base_cfg)
    c1.reset()
...
def test_published_uav_commands_include_unique_id():
    ...
    uav_cmds = [
        ...
        for d in cmds
        if d.get("cmd")
        in ("set_destination", "component.gimbal_tracking.set_orientation")
    ]
    assert uav_cmds, "expected at least one UAV control command in SEARCH"
    for d in uav_cmds:
        assert d.get("unique_id") == "20001", f"UAV command missing unique_id: {d}"
...
def test_rejected_decoy_detection_is_ignored_by_spatial_cooldown():
    ...
    state = _make_state(
        detected=True, sim_time=1.0, target_lat=27.0011, target_lon=125.0011
    )
    c.decide(state, dt=0.1)
    assert c.mode == "SEARCH"
...
def test_classifier_real_latches_track_active_flag():
    ...
    state = _make_state(
        detected=True, sim_time=2.0, target_type="ground_vehicle", misid_flag=False
    )
...
def test_search_sweep_avoids_recent_decoy_bearing():
    ...
    gimbal = next(
        cmd for cmd in cmds if cmd.cmd == "component.gimbal_tracking.set_orientation"
    )
...
def test_track_decoy_gate_spoofs_misid_before_fsm():
    c = _configured_controller()
    state = _make_state(
        detected=True, sim_time=2.0, target_type="decoy_vehicle", misid_flag=True
    )
    me = state.entities["20001"]
...
def test_decide_resets_fsm_state_on_decoy_cooldown_detection():
    ...
    c._decoy_cooldowns.append((27.001, 125.001, 10.0))
    state = _make_state(
        detected=True, sim_time=1.0, target_type="decoy_vehicle", misid_flag=True
    )
    c.decide(state, dt=0.1)
...
def test_committed_track_uses_standoff_destination_not_target_center():
    ...
    assert (
        abs(dest.params["latitude"] - 27.001) > 1e-5
        or abs(dest.params["longitude"] - 125.001) > 1e-5
    )
    assert dest.params["loiter_radius"] == c._track_orbit_radius_m
    assert any(cmd.cmd == "component.gimbal_tracking.set_orientation" for cmd in cmds)
