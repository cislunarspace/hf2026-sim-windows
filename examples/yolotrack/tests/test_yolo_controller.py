    ...
    *,
    ...
    return SimState(
        ...
        detection=Detection(
            ...
            target_position=GeoPosition(target_lat, target_lon, 0.0)
            if detected
            else None,
...
def force_track_mode(controller, target_position: GeoPosition | None = None):
...
class TestYoloTrackPath:
    ...
    def test_yolo_fresh_drives_gimbal(self):
        ...
        det = YoloDetection(
            ...
            tilt_delta=0.0,
            sim_time=1.0,
            wall_time_ms=int(__import__("time").time() * 1000),
        )
        ...
        state = make_state(
            detected=True, sim_time=1.0, uav_yaw=0.0, gimbal_pan=5.0, gimbal_tilt=-30.0
        )
        cmds = c.decide(state.without_truth(), 0.1)
    ...
    def test_yolo_stale_falls_back(self):
        ...
        stale_det = YoloDetection(
            ...
            tilt_delta=0.0,
            sim_time=1.0,
            wall_time_ms=0,  # epoch → 必然超 max_age
        )
...
class TestYoloGimbalMath:
    ...
    def test_yolo_offset_corrects_toward_image_center(self):
        ...
        state = make_state(
            detected=True, sim_time=1.0, uav_yaw=0.0, gimbal_pan=10.0, gimbal_tilt=-30.0
        )
    ...
    def test_yolo_offset_zero_keeps_target_stable(self):
        ...
        state = make_state(
            detected=True, sim_time=1.0, uav_yaw=0.0, gimbal_pan=0.0, gimbal_tilt=-30.0
        )
        # 连续 10 帧
        pans = []
        for _ in range(10):
        ...
        assert max(pans) - min(pans) < 1.0, f"pan 抖动过大: {pans}"
