...
def randomize_target(
    seed: int, center: dict | None = None, radius_deg: float | None = None
) -> dict[str, float]:
    ...
    rng.uniform(0, 2 * 3.141592653589793)
...
def summarize(results: list[RunMetrics]) -> dict[str, Any]:
    ...
    s = {
        ...
        "search_time_mean": (sum(search_times) / len(search_times))
        if search_times
        else None,
        "search_time_max": max(search_times) if search_times else None,
        "search_time_p95": (
            statistics.quantiles(search_times, n=20)[-1]
            if len(search_times) >= 20
            else (max(search_times) if search_times else None)
        ),
    ...
    *,
    ...
    proc = subprocess.run(
        [
            "python",
            "-m",
            "examples.uav_search_track_car.run",
            "--scenario",
            scenario_path,
            "--output",
            str(log_dir),
...
class BatchRunner:
    def __init__(
        ...
        *,
        ...
        self.scenario_path = (
            scenario_path or "examples/uav_search_track_car/config/scenario.json"
        )
