...
from search_track.swarm_controller import (
    SwarmController,
    _avoid_zone,
    _haversine_m,
    _nearest_edge_projection,
    _point_in_poly,
)
...
class AvoidZoneTest(unittest.TestCase):
    def test_pushes_outside(self):
        ...
        self.assertFalse(
            _point_in_poly(new_lat, new_lon, SQUARE),
...
class SwarmControllerAvoidanceTest(unittest.TestCase):
    ...
    def _zone(self, poly, alt_min=0.0, alt_max=2500.0):
        return ZoneView(
            type="air_defense", polygon=list(poly), alt_min=alt_min, alt_max=alt_max
        )
    ...
    def test_searches_when_uav_outside_zone(self):
        ...
        self.assertIsNotNone(
            self._dest_cmd(cmds), "sector search must always produce a waypoint"
        )
