...
def _state(
    ...
    for u in uavs:
        st.uavs[u["uid"]] = UavView(
            ...
            latitude=u.get("lat", 27.0),
            longitude=u.get("lon", 124.99),
...
class RunMetricsBasicTest(unittest.TestCase):
    ...
    def test_destroyed_counter(self):
        ...
        st = _state(10.0, [{"uid": "u1", "lat": 27.0, "lon": 124.99}], destroyed={"u1"})
        m.observe(st)
    ...
    def test_discovery_tracking(self):
        ...
        st = _state(5.0, [{"uid": "u1", "detected": True, "target_uid": "t1"}])
        m.observe(st)
        st = _state(6.0, [{"uid": "u1", "detected": True, "target_uid": "t2"}])
        m.observe(st)
                ...
                [
                    {
                        ...
                        "detected": True,
                        "misid_flag": misid,
                        "target_uid": "dec1" if misid else "t1",
                    }
                ],
            )
            m.observe(st)
    ...
    def test_sc001_discovery_time(self):
        ...
        for t in range(8):
            ...
            st = _state(
                float(t), [{"uid": "u1", "detected": True, "target_uid": f"t{t}"}]
            )
            m.observe(st)
    ...
    def test_sc005_handoff_max(self):
        ...
        for t in range(0, 6):
            st = _state(float(t), [{"uid": "u1", "detected": True, "target_uid": "t1"}])
            m.observe(st)
        for t in range(10, 13):
            st = _state(float(t), [{"uid": "u2", "detected": True, "target_uid": "t1"}])
            m.observe(st)
    ...
    def test_summarize_keys(self):
        ...
        expected = {
            ...
            "n_true_targets",
            "discovery_rate",
            "sc001_discovery_time_s",
            "tracking_ticks",
            "misid_ticks",
            "total_detected_ticks",
            "tracking_share",
            "misid_to_true_ratio",
            "sc005_handoff_max_s",
            "comm_sent_total",
            "comm_delivered_total",
            "auction_rounds",
            "auction_winners",
            "auction_conflict_count",
            "destroyed_events",
            "suspect_threat_points",
        }
