...
def uav(uid: str, name: str, lat: float, lon: float, heading: float) -> dict:
    return {
        ...
        "components": {
            ...
            "gimbal_tracking": {
                "type": "GimbalTrackingComponent",
                "enabled": True,
                "params": {
                    "pan_rate_limit_dps": 60.0,
                    "tilt_rate_limit_dps": 45.0,
                    "auto_track": False,
                    "fov": 30.0,
                },
            },
...
def target(
    uid: str, name: str, lat: float, lon: float, speed: float, wps: list
) -> dict:
    return {
        "id": uid,
        "name": name,
        ...
        "components": {
            "trajectory": {
                "type": "TargetTrajectoryComponent",
                "enabled": True,
...
def build() -> dict:
    ...
    for i in range(10):
        ...
        wps = [
            wp(lat, lon),
            wp(lat - 0.003, lon),
            wp(lat - 0.003, lon + 0.006),
            wp(lat, lon),
        ]
        targets.append(
            target(f"100{i + 1:02d}", f"target_{i + 1:02d}", lat, lon, speed, wps)
        )
