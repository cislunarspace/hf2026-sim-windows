...
from search_track.state import SwarmState, parse_swarm_state
...
from examples._common.redis_sub import connect_redis
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Spec 019 swarm-search kernel-integration runner"
    )
    p.add_argument(
    ...
    p.add_argument(
        "--start-sim", action="store_true", help="spawn opensim-sim as a subprocess"
    )
    p.add_argument(
        "--sim-binary",
        type=str,
...
def _inject_target_trajectories(
    redis_conn, state: SwarmState, trajectories: dict, *, dry_run: bool, log
) -> int:
    ...
    def is_known_uid(uid: str) -> bool:
        return uid in state.uavs or uid in state.targets or uid in state.decoys
...
def _summarise(st: SwarmState) -> dict:
    return {
        ...
        "n_tracking": sum(
            1 for u in st.uavs.values() if u.detected and not u.misid_flag
        ),
...
def _swarm_to_eval_inputs(state: SwarmState):
    ...
    uavs = [
        UavDetection(
            ...
            misid_flag=u.misid_flag,
            destroyed=u.destroyed,
            confidence=u.confidence,
        )
        for uid, u in state.uavs.items()
    ]
    true_targets = {uid: (t.latitude, t.longitude) for uid, t in state.targets.items()}
...
def _dry_run_seed_state() -> dict:
    ...
    for i in range(1, 11):
        s[f"200{i:02d}"] = uav(
            f"200{i:02d}", f"uav_{i:02d}", 27.00 + 0.002 * i, 124.985 + 0.002 * i
        )
    s["zones"] = {
        "air_defense": [
            {
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not args.dry_run:
            log(
                f"[run] connecting to Redis {args.redis_host}:{args.redis_port}, "
                f"subscribing to {args.redis_state_channel}"
            )
            r, pubsub = connect_redis(
                args.redis_host, args.redis_port, args.redis_state_channel
            )
        else:
        ...
        last_state: SwarmState | None = None
        sim_t0 = 0.0
        wall_t0 = time.time()
        ...
        sector_center_lat: float | None = None
        sector_center_lon: float | None = None
        ...
        try:
            ...
            _cfg = load_algorithm_config(str(EXAMPLE_DIR / "config" / "algorithm.yaml"))
        except Exception as e:  # pragma: no cover
            log(f"[run] WARN: algorithm config load failed ({e}); using defaults")
            _cfg = {}
        ...
        while True:
            ...
            if raw is not None:
                ...
                if not targets_activated:
                    n_activated = _inject_target_trajectories(
                        r, state, target_trajectories, dry_run=args.dry_run, log=log
                    )
                    if n_activated > 0:
                ...
                if self_uav is not None and self_uav.destroyed:
                    log(
                        f"[run] self-termination: {self_uid} destroyed at "
                ...
                if not controllers:
                    ...
                    sector_center_lat = (
                        _cfg.get("sector_center_latitude")
                        if hasattr(_cfg, "get")
                        else None
                    )
                    sector_center_lon = (
                        _cfg.get("sector_center_longitude")
                        if hasattr(_cfg, "get")
                        else None
                    )
                    if not sector_center_lat or not sector_center_lon:
                        pos = [v for v in state.uavs.values() if not v.destroyed]
                        if pos:
                        ...
                        log(
                            f"[run] sector center auto-filled from UAV "
                    ...
                    for idx, uid in enumerate(alive_uids):
                        ...
                        sc.set_sector_center(
                            float(sector_center_lat), float(sector_center_lon)
                        )
                        controllers[uid] = sc
                    log(
                        f"[run] built {n} SwarmController(s); "
                ...
                alive_now = sorted(u for u, v in state.uavs.items() if not v.destroyed)
                if len(alive_now) != len(controllers):
                    ...
                    for idx, uid in enumerate(alive_now):
                        ...
                        if sc is None:
                            ...
                            sc.configure(_cfg)
                            sc.set_sector_center(
                                float(sector_center_lat or 27.0),
                ...
                _alive_rate = (len(state.uavs) - len(_destroyed)) / max(
                    1, len(state.uavs)
                )
                ...
                if controllers:
                    ...
                    if cmds and not args.dry_run:
                        ...
                        for cmd in cmds:
                            r.publish(args.redis_command_channel, json.dumps(cmd))
                ...
                if state.sim_time - last_print_sim_t >= 1.0:
                    ...
                    log(
                        f"  t={state.sim_time:6.1f}  alive={s['n_alive']:2d}  "
            ...
            if sim_proc is not None and sim_proc.poll() is not None:
                log(
                    f"[run] opensim-sim exited (code {sim_proc.returncode}); "
        ...
        _final_alive_rate = (
            len(last_state.uavs)
            - len({u for u, v in last_state.uavs.items() if v.destroyed})
