...
if ! mkdir -p "$DIAG_PARENT" 2>/dev/null || ! touch "$DIAG_PARENT/.probe-$$" 2>/dev/null; then
    DIAG_PARENT="/tmp/opensim-diagnostics"
    mkdir -p "$DIAG_PARENT"
else
    rm -f "$DIAG_PARENT/.probe-$$"
fi
DIR="$DIAG_PARENT/opensim-diagnostics-$TS"
...
[ "${#PACK_ROOT}" -gt 120 ] && add_check WARN "包路径较长（${#PACK_ROOT} 字符）" "路径过长可能触发兼容问题，建议把包挪到更浅目录（如 ~/opensim）"
...
cap "$F" "OpenSim 相关环境变量" sh -c 'env | grep -E "^(OPENSIM_|REDIS_|WS_PORT|CAM_|PYTHON_BIN|NODE_PATH|UE_|MINGW_)" | sort'
...
if [ -n "$PSOUT" ]; then echo "$PSOUT" >> "$F"; else echo "  (无 OpenSim 相关进程在运行)" >> "$F"; fi
...
# 打包
ARCHIVE="$PACK_ROOT/opensim-diagnostics-$TS.tar.gz"
if tar -czf "$ARCHIVE" -C "$DIAG_PARENT" "opensim-diagnostics-$TS" 2>/dev/null; then
    echo ""
    echo "============================================================"
