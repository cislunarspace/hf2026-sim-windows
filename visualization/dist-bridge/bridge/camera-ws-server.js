...
class CameraWsServer {
    ...
    pushFrame(uid, frame) {
        ...
        if (!subs || subs.size === 0)
            return; // 无人订阅,跳过编码
        const msg = encodeFrameMessage(frame, uid);
        let sent = 0;
    ...
    async pushInitialFrame(ws, uid) {
        ...
        if (frame && ws.readyState === ws_1.WebSocket.OPEN) {
            ws.send(encodeFrameMessage(frame, uid));
        }
    }
    /** 节流统计日志:每 5 秒打一次推送量。 */
    maybeLogStats() {
...
function encodeFrameMessage(frame, uid) {
    ...
    const buf = Buffer.allocUnsafe(HEADER_LEN + uidBytes.length + frame.image.length);
    ...
    frame.image.copy(buf, HEADER_LEN + uidBytes.length);
    return buf;
}
