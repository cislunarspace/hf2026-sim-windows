...
try {
    ...
    $diagParent = Join-Path $env:TEMP 'opensim-diagnostics'
    New-Item -ItemType Directory -Force -Path $diagParent | Out-Null
}
$DIR = Join-Path $diagParent "opensim-diagnostics-$ts"
...
Write-Host ''
Write-Host '=== OpenSim 一键诊断 ===' -ForegroundColor Cyan
...
if ($PACK_ROOT.Length -gt 120) {
    Add-Check 'WARN' "包路径较长（$($PACK_ROOT.Length) 字符）" '超过 200 字符可能触发 MAX_PATH 问题，建议把包挪到更浅目录（如 D:\opensim）'
}
Capture $f '操作系统' {
...
Capture $f 'Windows 事件日志：应用程序错误（近 7 天，OpenSim 相关）' {
...
# 打 zip
if (-not $NoZip) {
    $zipPath = Join-Path $PACK_ROOT "opensim-diagnostics-$ts.zip"
    try {
