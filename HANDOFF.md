...
| 优先级 | 任务 | 备注 |
|---|---|---|
| 高 | 赛题三 proximity 根治（首局 32 次罚满） | 双僚机 300/500m 圈按 uid 哈希只有两档，两僚机可能同圈（最近 <200m）；需用 C: 消息协商半径（先到先得占 300m 圈，后到 500m 圈）；10 机 SEARCH 扇区交界也需复核 |
| 高 | 赛题三 SAM 击毁归因（首局死 2 机） | 查死亡位置：推出 bbox 后 300/500m 盘旋圈局部压进真实 SAM 区？还是出生点在区内？SAM 内目标当前策略是保 alive 弃杀，需确认推避在所有飞行阶段生效 |
...
| 高 | 赛题二/三 5 m/s 档判别 | 赛题二 1 真 + 15 诱饵、赛题三 4 真 + 20 诱饵同速档，速度带不可用。方向：停顿模式（真目标路线 Start 有 WaitTime 30s 停顿，诱饵恒 5 m/s 无停顿）——但仅开局 30s 窗口有效；验证集目标还有躲避能力（官方描述） |
| 中 | 赛题一 RMSE 压线优化 | 新路线回退局 59.84 压线（RMSE 20.6 vs 晴天 12.5），方向：回退螺旋的搜索-截获衔接（检出后 EKF 重新初始化路径）、上报位置偏向检测加权 |
| 中 | 赛题二/三多种子均值 | 单局方差极大（0~17.9），任何策略改动需 5+ 局均值才有结论；跑前确认无残留引擎（run_fast 已自动清理） |
