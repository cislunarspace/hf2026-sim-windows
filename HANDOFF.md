...
| 赛题三 | 未跑 | 骨架已按 K=3 重写（coop 底座移植），未跑仿真 | 10 机：扇区螺旋 + SAM 推避 + 双僚机槽位 + C:/J: 消息分离；判别放弃 5 m/s 档（4 真+20 诱饵运动不可分），杀其余 7 个即过 kill 线。已知风险见 HANDOFF §6 |
...
| 优先级 | 任务 | 备注 |
|---|---|---|
| 高 | 赛题三仿真验证（K=3 重写后首跑） | 关注：双僚机槽位仲裁、SAM 推避有效性（alive）、协锁判毁需 2 名队友、proximity（两僚机可能哈希到同一盘旋圈）；5 m/s 档放弃意味着 kill 上限 0.7=压线，任何一架非 5 档漏杀即不过线 |
