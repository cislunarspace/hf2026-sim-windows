...
def _resolve_redis_port(scenario: str, scenario_json: str | None) -> int:
    ...
    try:
        ...
        print(
            f"[cli] WARNING: 读取 {sj_path} 的 redis_port 失败,回退 6379: {e!r}",
            file=sys.stderr,
            flush=True,
        )
    return 6379
...
def _load_agent_class(spec: str):
    ...
    if mod is None:
        raise ImportError(
            f"could not import {module_path!r} (or competition.{module_path})"
        )
...
def main(argv=None) -> int:
    p = argparse.ArgumentParser(
        prog="competition", description="Competition Agent SDK runner"
    )
    ...
    run_p.add_argument("--agent", required=True, help="module.path:ClassName")
    ...
    run_p.add_argument(
        "--viz-dir",
        default=None,
        help="path to the visualization/ folder (default: <repo>/visualization)",
    )
    run_p.add_argument(
        "--no-browser",
        action="store_true",
    ...
    run_p.add_argument(
        ...
        action="store_true",
        default=True,
        help="spawn opensim-sim (default)",
    )
    run_p.add_argument(
        "--no-start-sim",
        dest="start_sim",
    ...
    run_p.add_argument(
        "--dry-run", action="store_true", help="no Redis / no engine (synthetic loop)"
    )
    run_p.add_argument(
        "--mode",
        choices=["train", "eval"],
    ...
    run_p.add_argument(
        "--yolo-model", default="", help="YoloDetector 模型路径 (eval 模式)"
    )
    ...
    run_p.add_argument(
        ...
        type=int,
        default=None,
        help="Redis port (default: read from scenario.json or 6379)",
    )
    ...
    if args.noise_sigma < 30.0:
        print(
            f"[cli] --noise-sigma {args.noise_sigma} below 30m floor; clamped to 30.0"
        )
    ...
    viz_kwargs = dict(
        visualize=args.visualize, viz_dir=args.viz_dir, open_browser=not args.no_browser
    )
    if args.scenario == "search_track":
        ...
        run(
            agent_cls,
            duration=args.duration or 600.0,
            seed=args.seed,
            mode=args.mode,
