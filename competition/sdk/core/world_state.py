...
class WorldState:
    ...
    def targets(self) -> dict[str, EntityTruth]:
        return {u: e for u, e in self.entities.items() if e.kind == "ground_vehicle"}

    @property
    def decoys(self) -> dict[str, EntityTruth]:
        return {u: e for u, e in self.entities.items() if e.kind == "decoy_vehicle"}
...
def parse_world_state(raw: dict[str, Any]) -> WorldState:
    ...
    for key, val in raw.items():
        ...
        kind = _normalise_kind(
            str(val.get("type", "")), str(plat.get("entity_type", ""))
        )
        ...
        for z in zones_obj.get(kind_key, []) or []:
            st.zones.append(
                ZoneTruth(
                    kind=kind_key,
                    polygon=[
                        (float(p[0]), float(p[1])) for p in (z.get("polygon", []) or [])
                    ],
