...
class AccuracySimulator(BaseDetector):
    ...
    def detect(
        ...
        if not truth.detected:
            return [
                Detection(detected=False, confidence=0.0, target_type=truth.target_type)
            ]
        ...
        if self.effective_noise_sigma_m > 0 and lat is not None and lon is not None:
            d_lat = self._rng.gauss(
                0, meters_to_deg(self.effective_noise_sigma_m, lat, False)
            )
            d_lon = self._rng.gauss(
                0, meters_to_deg(self.effective_noise_sigma_m, lat, True)
            )
            lat += d_lat
            lon += d_lon
...
class YoloDetector(BaseDetector):
    ...
    def detect(
        ...
        tlat, tlon = pan_tilt_to_latlon(
            uav_lat=obs.self.lat,
            uav_lon=obs.self.lon,
            uav_alt=obs.self.alt,
            gimbal_pan=obs.self.gimbal_pan,
            gimbal_tilt=obs.self.gimbal_tilt,
            pan_delta=yd.pan_delta,
            tilt_delta=yd.tilt_delta,
        )
        return [
            Detection(
                detected=True,
                confidence=float(yd.confidence),
                target_lat=tlat,
                target_lon=tlon,
