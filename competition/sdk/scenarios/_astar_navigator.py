...
使用引擎端的 C++ PathPlanner::findPath 规划路线。流程：
1. Python 发 astar_plan 命令（指定起终点）
2. 引擎执行 A*，结果写入 entity_state["astar_plan_result"]
3. Python 读取结果，用 set_trajectory 一次性发给车辆
...
def make_route_progress_cb(client, total_segs: int, log=print):
    """构造一个段级进度回调, 把"规划车辆路线"进度发到 sim:progress 频道。

    三个赛题 runner 的 inject_startup 调用本函数, 传入 *所有实体合计* 的
    路线段总数 total_segs; 返回的闭包由 _plan_full_route 在每段规划完后
    调用 ``cb(done_segs_in_entity, total_segs_in_entity, uid)``。

    线程安全的 per-uid 绝对计数: 闭包维护 ``done_by_uid: {uid: int}``,
    每次调用直接把 ``done_in_entity`` 赋值给本 uid 的键 (绝对值, 非自增)。
    全局已完成段数 = ``sum(done_by_uid.values())``, 映射到 0.51-1.0 的 pct。
    由于每个 uid 只由规划该实体的线程写入 (CPython dict 键赋值是原子的),
    即便闭包被多个实体/线程共享 (如 inject_startup_concurrent) 也不会丢更新。

    total_segs <= 0 时返回 no-op (无可规划段, 不发进度)。
    """
    if total_segs <= 0:
    ...
    def cb(done_in_entity: int, total_in_entity: int, uid: str) -> None:
        ...
        # 避免多线程共享闭包时的丢更新竞态。total_in_entity 为本实体总段数,
        # 保留在签名中以便未来按实体细分进度; 当前仅用 done_in_entity。
        ...
        payload = {
            ...
            "phase": "规划车辆路线",
            "pct": round(pct, 4),
            "detail": f"{global_done}/{total_segs} 段",
        }
...
# 约 260 段全串行 → 启动耗时 30~40s。本入口把"每辆车调一次 inject_astar_*"
# 并发化: 不同实体相互独立可并发; 实体内部段间仍串行 (保持 plan_result_
# 按 entity 存储 + 段间拓扑依赖的正确性)。
