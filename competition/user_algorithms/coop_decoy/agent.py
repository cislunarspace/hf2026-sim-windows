...
算法：
  - Imm 滤波器（Rust，CV+CA+CT）用于 VERIFY 速度判别
  - 螺旋搜索（uid 扇区分配）
  - 盘旋跟踪 + 广播协同
  - K=2 同时盯防 20s 摧毁
"""
...
_VERIFY_FRAMES = 80        # VERIFY 窗口帧数（8s @ 10Hz）
_VERIFY_SPEED_THRESH = 3.9  # 速度阈值（m/s）
...
class CoopDecoyAgent(CoopAgent):
    ...
    def _do_search(self, obs: CoopObs, dt: float) -> List[Command]:
        ...
        if self._shared_target is not None:
            near_decoy = any(
                haversine_m(self._shared_target[0], self._shared_target[1],
                            d[0], d[1]) < 150.0
    ...
    def _do_verify(self, obs: CoopObs, dt: float) -> List[Command]:
        ...
        if det.detected and det.target_lat is not None and det.target_lon is not None:
            ...
            if self._imm is not None:
                if not self._imm.is_initialized():
                    self._imm.initialize(obs.self.lat, obs.self.lon,
                                         bearing, range_m)
                else:
        ...
        if self._shared_target is not None and self._target is not None:
            d = haversine_m(self._shared_target[0], self._shared_target[1],
        ...
        if self._verify_frames >= _VERIFY_FRAMES:
            speed = self._imm.speed_mps() if self._imm and self._imm.is_initialized() else 0.0
        ...
        # 飞向目标区域
        if self._target:
            cmds.append(fly_to(self._target[0], self._target[1],
    ...
    def _do_track(self, obs: CoopObs, dt: float) -> List[Command]:
        ...
        if det.detected and det.target_lat is not None and det.target_lon is not None:
            if self._target:
                d = haversine_m(det.target_lat, det.target_lon,
                                self._target[0], self._target[1])
                if d < 250.0:
                    self._target = (det.target_lat, det.target_lon)

        # 盯防计时
        tracking = (det.detected and det.target_lat is not None and self._target
                    and haversine_m(det.target_lat, det.target_lon,
                                    self._target[0], self._target[1]) < 250.0)
        if tracking:
            gap = self._sim_time - self._last_det_time
            if self._dwell_time > 0 and gap <= _TRACK_GRACE_S + dt:
                self._dwell_time += dt
        ...
        # 摧毁或超时 → SEARCH
        if self._dwell_time >= _TRACK_DWELL_S or self._track_time >= _TRACK_TIMEOUT_S:
            if (self._dwell_time < _TRACK_DWELL_S and self._target
                    and self._imm and self._imm.is_initialized()
        ...
        if self._target:
            ...
            cmds.append(fly_to(self._target[0], self._target[1],
                               alt=_SEARCH_ALT, speed=_TRACK_SPEED,
    ...
    def _do_join(self, obs: CoopObs, dt: float) -> List[Command]:
        ...
        # 到达目标附近 → 切换到 TRACK
        dist_to_target = haversine_m(obs.self.lat, obs.self.lon,
                                      self._target[0], self._target[1])
