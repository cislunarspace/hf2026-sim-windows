...
def _uid_partition(uid: str) -> tuple[float, float]:
    ...
    if uid.isdigit():
        ...
        idx = (
            int(tail) % n
            if tail.isdigit()
            else (int(hashlib.md5(uid.encode()).hexdigest(), 16) % n)
        )
    else:
...
class _EMATracker:
    ...
    def speed_mps(self, tick_hz: float = 10.0) -> float:
        ...
        sxy = sum(t * la for t, la in zip(ts, lats, strict=False))
        denom = ns * sxx - sx * sx
        if abs(denom) < 1e-20:
            return 0.0
...
class CoopDistributedAgent(CoopAgent):
    ...
    def configure(self, config) -> None:
        ...
        self._verify_speed_confirm: float = (
            3.0  # m/s → real target (decoy P95≈4.5 but median 1.8)
        )
    ...
    def decide(self, obs: CoopObs, dt: float) -> list[Command]:
        ...
        if self._state == self.TRACK:
            ...
            if tracking:
                ...
                if (
                    self._dwell_time > 0 and gap <= self._dwell_grace + dt
            ...
            # destroy / timeout
            if (
                self._dwell_time >= self._dwell_target
                or self._track_t >= self._track_timeout
            ):
